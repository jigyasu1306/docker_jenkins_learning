...
size = 2
haar_cascade = cv2.CascadeClassifier(r'C:\IIIT\Minor\SEM6\Facial-Recognition-for-Crime-Detection-master\face_cascade.xml')
...
def recognize_face(model, frame, gray_frame, face_coords, names):
    ...
    for i in range(len(face_coords)):
        ...
        if (confidence<95 and names[prediction] not in recog_names):
            ...
            #cv2.putText(frame, f"{names[prediction]}", (x,y), font, 1, color, 2, cv2.LINE_AA)
        elif (confidence >= 95):
